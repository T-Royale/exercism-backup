...
size_t pos_student_in_roster(roster_t *actual, const char *name) {
    for(size_t i = 0; i < actual->count; i++) {
        if(strcmp(name, actual->students[i].name) == 0) {
            return i; // Se encontró al estudiante, devolver la posición
        }
    }
    // Si el estudiante no se encuentra, devolver SIZE_MAX
    return SIZE_MAX;
}
