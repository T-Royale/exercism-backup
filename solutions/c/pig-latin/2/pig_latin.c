...
int translate_word(char word[MAX_WORD_LEN]){
	...
	if(is_vowel(word[0]) || strncmp(word, "xr", 2) == 0 || strncmp(word, "yt", 2) == 0){
		...
		snprintf(buff, sizeof(buff), "%say", tmp);
	} else {
		...
		if(has_qu){
			snprintf(buff, sizeof(buff), "%s%.*squay", pos, prefix_len-2, tmp);
		} else if(has_y){
			snprintf(buff, sizeof(buff), "y%s%.*say", pos, prefix_len-1, tmp);
		} else {
			snprintf(buff, sizeof(buff), "%s%.*say", pos, prefix_len, tmp);
		}
	}
	strncpy(word, buff, MAX_WORD_LEN);
	return 0;
}
